...
export default function JoinTeam() {
  ...
  const fetchUsers = async () => {
    ...
    const usersWithoutTeam = allUsers.filter(u => !u.teamId && u.uid !== user.uid);
    ...
    try {
      if (currentUserData.teamId) {
        const proceed = window.confirm(
          "You are already in a team. Do you want to join another team anyway?"
        );
        if (!proceed) {
          navigate("/dashboard");
          return;
      ...
      // Determine teamId: use selectedUser.id if they don't have a team yet
      ...
      // **Create a team document in Firestore if it doesn't exist yet**
      ...
      setMessage(`You have successfully joined ${selectedUser.displayName}'s team!`);
  ...
  const leaveTeam = async () => {
    ...
    try {
      ...
      const snapshot = await getDocs(usersRef);
      const allUsers = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      const teammates = allUsers.filter(u => u.teamId === currentUserData.teamId);
      ...
      setMessage("You have left your team.");
      await fetchUsers();
  ...
  if (loading) return <p className="text-green-800 text-center mt-10">Loading users...</p>;

  return (
    <div className="flex flex-col items-center min-h-screen bg-green-100 p-6">
      <h2 className="text-3xl sm:text-4xl font-extrabold text-green-900 mb-6">Join a Team</h2>
      ...
      <div className="w-full max-w-md">
        ...
        {users.length === 0 ? (
          <p className="text-green-900 text-center mt-4">No users available to join.</p>
        ) : (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    <p className="font-semibold text-green-900 text-lg">{u.displayName}</p>
