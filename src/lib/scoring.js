...
export function strokesReceivedForHole(handicap, holeStrokeIndex) {
  // Supports positive (receives strokes), zero (scratch), and negative (plus handicap gives strokes back)
  const numericHandicap = Number(handicap);
  if (!Number.isFinite(numericHandicap) || !holeStrokeIndex) return 0;
  if (numericHandicap === 0) return 0;

  const absValue = Math.abs(numericHandicap);
  const base = Math.floor(absValue / 18);
  const remainder = absValue % 18;
