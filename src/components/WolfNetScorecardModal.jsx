...
export default function WolfNetScorecardModal({ game, onClose }) {
  ...
  const holeRenderInfo = (absIndex) => {
    ...
    const [pA, pB] = others;
    const wolfScore = getNet(wolf.userId, absIndex);
    const aScore = getNet(pA.userId, absIndex);
    const bScore = getNet(pB.userId, absIndex);
    ...
    let partnerId = null;
      ...
      if (decision === "blind" || decision === "lone") {
        ...
        if (
          ...
          } else if (oppBest < wolfScore) {
            if (aScore === oppBest) highlightIds.add(pA.userId);
            if (bScore === oppBest) highlightIds.add(pB.userId);
          }
        }
        ...
        if (partner && solo) {
          ...
          const teamBest =
            wolfScore != null && getNet(partner.userId, absIndex) != null
              ? Math.min(wolfScore, getNet(partner.userId, absIndex))
              : null;
          const soloScore = getNet(solo.userId, absIndex);
          if (teamBest != null && soloScore != null) {
            if (teamBest < soloScore) {
              if (wolfScore === teamBest) highlightIds.add(wolf.userId);
              if (getNet(partner.userId, absIndex) === teamBest)
                highlightIds.add(partner.userId);
            } else if (soloScore < teamBest) {
  ...
  const calculateHolePointsMap = (holeIndex) => {
    ...
    const [pA, pB] = others;
    const wolfScore = getNet(wolfId, holeIndex);
    const aScore = getNet(pA.userId, holeIndex);
    const bScore = getNet(pB.userId, holeIndex);
    ...
    if (typeof decision === "string") {
      ...
      const partnerScore = getNet(partner.userId, holeIndex);
      const soloScore = getNet(solo.userId, holeIndex);
      if (partnerScore == null || soloScore == null) return {};
  ...
  const handleShare = async () => {
    ...
    setIsSharing(true);
    try {
      const filename = `${game.name} - Wolf Scorecard (Handicaps)`;
            ...
            <tbody>
              {Array.from({ length: holeCount }).map((_, displayIndex) => {
                ...
                const pointsEarned = (() => {
                  ...
                  if (others.length !== 2) return [];
                  const [pA, pB] = others;
                  const wolfScore = getNet(wolfId, i);
                  const aScore = getNet(pA.userId, i);
                  ...
                  if (decision === "blind") {
                    ...
                    const solo = partner && partner.userId === pA.userId ? pB : pA;
                    if (!partner || !solo) return [];
                    const teamBest = Math.min(wolfScore, getNet(partner.userId, i) ?? Infinity);
                    const soloScore = getNet(solo.userId, i);
                    if (teamBest < soloScore) {
                      return [
                        { userId: wolfId, pts: 1 },
                ...
                const tieLabel = (() => {
                  ...
                  const [pA, pB] = others;
                  const w = getNet(wolfId, i);
                  const a = getNet(pA.userId, i);
                  const b = getNet(pB.userId, i);
                  ...
                  if (decision === "blind" || decision === "lone") {
                    ...
                    const teamBest = Math.min(w, getNet(partner.userId, i) ?? Infinity);
                    const soloScore = getNet(solo.userId, i);
                    if (teamBest === soloScore) return "Tie";
                  }
                  return null;
                })();
                ...
                const getPointsTooltip = () => {
                  ...
                  if (decision === "blind") {
                    const oppBest = Math.min(a, b);
                    if (w < oppBest) {
                      return `${wolfName} (${w}) beat Opponents' Best (${oppBest}) [Blind Lone Wolf, Net]: ${wolfName} +6`;
                    } else if (w > oppBest) {
                    ...
                    const oppBest = Math.min(a, b);
                    if (w < oppBest) {
                      return `${wolfName} (${w}) beat Opponents' Best (${oppBest}) [Net]: ${wolfName} +3`;
                    } else if (w > oppBest) {
                      return `Opponents' Best (${oppBest}) beat ${wolfName} (${w}) [Net]: ${aName} +1, ${bName} +1`;
                    } else {
                      return `${wolfName} (${w}) tied Opponents' Best (${oppBest}) [Net]: ${wolfName} +1`;
                    }
                  } else if (typeof decision === "string") {
                    const partnerId = decision;
                    const partner = partnerId === pA.userId ? pA : partnerId === pB.userId ? pB : null;
                    ...
                    const partnerScore = getNet(partner.userId, i);
                    const teamBest = Math.min(w, partnerScore ?? Infinity);
                    const soloScore = getNet(solo.userId, i);
                    if (teamBest < soloScore) {
                      return `Team Best (${teamBest}, from ${wolfName} ${w} or ${partnerName} ${partnerScore}) beat ${soloName} (${soloScore}) [Net]: ${wolfName} +1, ${partnerName} +1`;
                    } else if (soloScore < teamBest) {
                      return `${soloName} (${soloScore}) beat Team Best (${teamBest}) [Net]: ${soloName} +3`;
                    } else {
                      return `Team Best (${teamBest}) tied ${soloName} (${soloScore}) [Net]: ${soloName} +1`;
                    }
                  }
                  return "";
                };
                return (
                  <tr key={i} className="border-t border-blue-500 dark:border-blue-400">
                    ...
                    {(() => {
                      ...
                      const netScore = getNet(p.userId, i);
              ...
              <tr className="border-t-2 border-blue-500 dark:border-blue-400 bg-gray-100 dark:bg-gray-800">
                ...
                {displayPlayers.map((player, idx) => {
                  ...
                  for (let h = startIndex; h < startIndex + holeCount; h++) {
                    const net = getNet(player.userId, h);
                    const g = getGross(player.userId, h);
                    if (typeof net === "number" && net > 0) netTotal += net;
