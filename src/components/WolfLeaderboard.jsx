...
export default function WolfLeaderboard({ game }) {
  const { players = [], wolfOrder = null, wolfDecisions = [] } = game || {};
  ...
  const computeWolfTotals = useMemo(() => {
    ...
    for (let i = startIndex; i < holesEnd; i++) {
      ...
      const wolfGross = getGrossFor(wolfPlayer, i);
      const aGross = getGrossFor(pA, i);
      const bGross = getGrossFor(pB, i);

      // Count as "thru" if at least one of the three has a score entered
      if (wolfGross != null || aGross != null || bGross != null) {
        thru++;
      }
      ...
      if (decision === "blind") {
        ...
        const teamBest = Math.min(wolfGross, getGrossFor(partner, i) ?? Infinity);
        const soloGross = getGrossFor(solo, i);
        if (teamBest < soloGross) {
          totals.set(wolfId, (totals.get(wolfId) || 0) + 1);
          totals.set(partner.userId, (totals.get(partner.userId) || 0) + 1);
        } else if (teamBest > soloGross) {
    ...
    return { totals, thru };
  }, [players, effectiveWolfOrder, wolfDecisions, wolfHoles, startIndex, holesEnd, exactlyThree]);
  ...
  return (
    <div>
      ...
      {modalOpen && selectedPlayer && (
        <WolfScorecardModal game={game} onClose={closeModal} />
      )}
    </div>
  );
}
